console.log('We are being very pedantic here, but this will walk through a very simple neural network.')
console.log('Begin with one parameter, attempt to solve problem that can fit a linear model')
console.log('here is our first set of data, representing some pattern we want to recognize\n')
...
console.log('So, can we get a model to adjust w1 to 3? let\'s start with a random weight.')
...
console.log('As expected, it\'s usually not great. By chance one or two or even all the answers could be correct, but its rare')
console.log( 'We need to correct w1 when it\'s wrong, and see if we get closer. Press enter when you\'re ready')
...
console.log('We need a better cost function! Cost functions are used to determine the error of the model')
console.log('A common one is to square the difference - this means that the error cannot be negative which is nice for simple math')
console.log('Let\'s figure out how wrong we are, on average, and then adjust our weight to see if we\'re closer')
await Deno.stdin.read(new Uint8Array(1))
...
console.log('The first loop can be called some measure of how bad at prediction we are')
...
console.log('what we really want to do is minimize the cost function of our model by adjusting w1')
console.log('this could get complicated, involving the derivative of some function')
console.log('But we\'re writing Typescript, not here to be nerds (lol)')
console.log('the definition of derivative is the limit as h approaches 0 of the formula (f(x+h) - f(x)) / h')
console.log('let\'s call h something small, like 0.001, and that\'s close enough')
const h = 0.001
...
console.log('Well, we wanted 3 and started with 6.14159, but the next guess is -172!!')
console.log('We overshot the correct answer. Let\'s just multiply the result of our redneck derivative by some tiny fraction')
await Deno.stdin.read(new Uint8Array(1))
...
console.log('now we\'re taking a small step towards the correct answer! This is great. let\'s do that some more')
await Deno.stdin.read(new Uint8Array(1))
...
console.log('this is because we\'ve reached the limits of a single parameter model')
console.log('In order to fit a linear pattern that doesn\'t pass through the origin, we need a bias parameter')
console.log('let\'s call that w0')
let weights = [ 1.771, 6.14159 ]

console.log('We will need a new cost function that takes an array of weights instead of just one')
...
epochs = 50
for (i = 0; i < epochs; i++) {
  const deltaw0 = (betterCost([weights[0] + h, weights[1]], training_set_linear) - betterCost(weights, training_set_linear)) / h * learning_rate
  const deltaw1 = (betterCost([weights[0], weights[1] + h], training_set_linear) - betterCost(weights, training_set_linear)) / h * learning_rate
...
console.log('This happens sometimes, so let\'s just pick new initial conditions and try again!')
await Deno.stdin.read(new Uint8Array(1))
...
console.log('The deltaw0 and deltaw1 are approximating a gradient, and of course we used a fixed distance instead of a derivative')
console.log('This is also not optimized - backpropagation exists')
console.log('But this is the basics!')

console.log('All thanks to tsoding over at twitch.television for inspiration: https://(www.twitch.tv//tsoding')
